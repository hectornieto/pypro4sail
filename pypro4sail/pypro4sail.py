...
def surface_emissivity(lai, leaf_angle, view_zenith=0, e_leaf=0.99, e_soil=0.97,
                       chunk_size=1e6):
    ''' Estimates the broadband at-sensor thermal radiance using 4SAIL model.
    ...
    rsoil = 1. - e_soil
    rho_leaf = 1. - e_leaf
    tau_leaf = np.zeros(rsoil.shape)
    # Calculate the lidf,
    n_elements = lai.size
    n_subset = int(np.ceil(n_elements / chunk_size))
    emiss = np.zeros(n_elements)
    print("Computing emissivity, progress:", end="")
    for id_subset in range(n_subset):
        pos_ini = int(id_subset * chunk_size)
        print(" %.1f %%"%(100 * pos_ini / n_elements), end="...")
        pos_end = int(np.minimum((id_subset + 1) * chunk_size, n_elements))
        lidf = four_sail.calc_lidf_campbell_vec(leaf_angle[pos_ini:pos_end])
        lai_subset = lai[pos_ini:pos_end]
        # 4SAIL for canopy reflectance and transmittance factors
        [_, _, _, _, _, _, _, _,
         _, _, _, _, _, _, rdot, *_] = four_sail.foursail_vec(lai_subset,
                                                              np.zeros(lai_subset.shape),
                                                              lidf,
                                                              np.zeros(lai_subset.shape),
                                                              view_zenith[pos_ini:pos_end],
                                                              np.zeros(lai_subset.shape),
                                                              rho_leaf[:, pos_ini:pos_end],
                                                              tau_leaf[:, pos_ini:pos_end],
                                                              rsoil[:, pos_ini:pos_end])

        del _, lidf, lai_subset
        # Estimate the apparent surface directional emissivity
        emiss[pos_ini:pos_end] = 1.0 - rdot
        del rdot
